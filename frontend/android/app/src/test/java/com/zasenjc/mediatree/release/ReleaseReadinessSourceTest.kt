...
class ReleaseReadinessSourceTest {
    ...
    fun changelogsExposeCurrentAndroidVersionSectionForReleaseNotes() {
        ...
        assertTrue(english.contains("## 0.1.03"))
        assertTrue(chinese.contains("## 0.1.03"))
        assertTrue(english.contains("Updated Android `versionCode` to `4` and `versionName` to `0.1.03`"))
        assertTrue(chinese.contains("将 Android `versionCode` 更新为 `4`，`versionName` 更新为 `0.1.03`"))
        assertFalse(english.substringBefore("## 0.1.03").contains("Updated Android `versionCode`"))
        assertFalse(chinese.substringBefore("## 0.1.03").contains("将 Android `versionCode` 更新为"))
    }
}
