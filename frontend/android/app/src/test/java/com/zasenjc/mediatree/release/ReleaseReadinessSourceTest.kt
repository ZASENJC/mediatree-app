...
class ReleaseReadinessSourceTest {
    ...
    fun changelogsExposeCurrentAndroidVersionSectionForReleaseNotes() {
        ...
        assertTrue(english.contains("## 0.1.09"))
        assertTrue(chinese.contains("## 0.1.09"))
        assertTrue(english.contains("Updated Android `versionCode` to `10` and `versionName` to `0.1.09`"))
        assertTrue(chinese.contains("将 Android `versionCode` 更新为 `10`，`versionName` 更新为 `0.1.09`"))
        assertFalse(english.substringBefore("## 0.1.09").contains("Updated Android `versionCode`"))
        assertFalse(chinese.substringBefore("## 0.1.09").contains("将 Android `versionCode` 更新为"))
    }
    ...
    fun releaseNotesBlockStaysUserFacingAndFunctionalOnly() {
        val english = appRoot.resolve("../../../CHANGELOG.md").readText()
        val chinese = appRoot.resolve("../../../CHANGELOG_zh-CN.md").readText()
        val englishReleaseNotes = releaseNotesBlock(english, "0.1.09")
        val chineseReleaseNotes = releaseNotesBlock(chinese, "0.1.09")

        assertTrue(englishReleaseNotes.contains("Playback resume is reliable again"))
        assertTrue(englishReleaseNotes.contains("Watched flags now appear consistently"))
        assertTrue(englishReleaseNotes.contains("Exiting fullscreen after the screen has been off"))
        assertTrue(chineseReleaseNotes.contains("播放记忆跳转恢复稳定"))
        assertTrue(chineseReleaseNotes.contains("“已观看”插旗现在会一致显示"))
        assertTrue(chineseReleaseNotes.contains("长时间息屏后从全屏播放退出"))
