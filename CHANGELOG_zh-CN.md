...
## 0.1.09 (2026-06-16) — 播放记忆与已观看标记修复

<!-- release-notes:start -->
- 播放记忆跳转恢复稳定：视频会等待原生播放器准备好后，再跳到上次记忆位置
- “已观看”插旗现在会一致显示在首页、详情页剧集、浏览页海报和源文件名文件夹浏览中
- 长时间息屏后从全屏播放退出，现在会正确恢复应用顶部/底部栏和系统状态栏
<!-- release-notes:end -->

### Android App

- 将 Android `versionCode` 更新为 `10`，`versionName` 更新为 `0.1.09`
- 统一 watched/favorite 标签判断，MediaTree、Jellyfin 和 Emby 元数据使用同一套大小写不敏感逻辑
- 移除 mpv `loadfile start=...` 恢复逻辑，改为播放器准备就绪后再执行播放记忆 seek
- 新增已观看插旗、播放记忆 seek、mpv 加载命令、全屏生命周期恢复和详情页进度同步回归覆盖

## 0.1.08 (2026-06-16) — 播放器控制打磨

<!-- release-notes:start -->
- 播放器控制层不再让整块视频画面变暗，只有可见控制行保留轻微渐隐
- 双击快退/快进、长按快进和横向滑动进度时，现在只显示小提示窗，不再展开完整播放器控制栏
<!-- release-notes:end -->

### Android App

- 将 Android `versionCode` 更新为 `9`，`versionName` 更新为 `0.1.08`
- 为播放器手势提示和局部控制行渐隐新增源码回归覆盖

## 0.1.07 (2026-06-16) — M3U 直播与首页源文件名显示

<!-- release-notes:start -->

- 设置页现在支持添加、编辑、切换和移除 M3U 订阅配置
- M3U 频道现在会显示在首页，支持搜索、缓存刷新、台标、分组标签和频道收藏
- M3U 收藏频道现在有独立收藏页，并可直接进入原生播放器
- M3U 播放现在使用原生播放器，支持直连直播流和播放器内频道切换
- 首页现在可以在保留海报网格的同时显示原始文件夹和文件名
- 从首页进入的源文件名文件夹浏览在返回时会回到首页
- 详情页剧照现在使用紧凑的多行小网格展示
- 详情页剧照预览复用全屏图片查看器的缩放和平移手势
<!-- release-notes:end -->

### Android App

- 将 Android `versionCode` 更新为 `8`，`versionName` 更新为 `0.1.07`
- 新增 M3U 订阅解析、缓存持久化、收藏持久化、配置切换和直连播放路由
...
## 0.1.05 (2026-06-08) — 剧照查看与播放器全屏控制

<!-- release-notes:start -->
- 详情页剧照现在可以点开查看，并支持左右滑动切换
- 播放器音量、亮度、倍速和进度提示更轻量，显示在画面上方
- 全屏播放新增旋转按钮，可快速切换竖屏/横屏
<!-- release-notes:end -->

### Android App

- 将 Android `versionCode` 更新为 `6`，`versionName` 更新为 `0.1.05`
- 详情页剧照支持点开进入图片查看，并可左右滑动切换剧照
- 播放器音量、亮度、倍速和滑动进度提示改为透明底、小字号，并移动到画面上方
- 播放器提示文案改为简短格式，例如倍速只显示 `2.0x`
- 全屏播放器在全屏按钮旁新增旋转按钮，可快速切换竖屏/横屏全屏
- 新增 repo-local `.agents/skills` 与 `.codex/agents` 配置，支持 Codex 在本仓库使用只读 multi-agent 角色辅助审查与规划

---

## 0.1.04 (2026-06-07) — 首页媒体库排序与网格修复

### Android App

- 将 Android `versionCode` 更新为 `5`，`versionName` 更新为 `0.1.04`
- 修复首页媒体库网格被 app 侧手动截断的问题，确保所有媒体库海报都交给 `LazyVerticalGrid` 渲染
- 修复首页排序偏好在重启后被默认排序抢跑覆盖的问题，用户手动选择的排序会继续作为后续默认排序
